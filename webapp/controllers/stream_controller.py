...
@socketio.on('broadcast_video')
def handle_broadcast_video(data):
    print("Received 'broadcast_video' event from broadcaster with data:", data)
    if 'stream' in data:
        print("Stream data received:", data['stream'])
    else:
        print("No stream data received.")
    # Process the video stream (e.g., store it, distribute it to viewers)
    # Emit the stream to viewers
    emit('view_video', data, broadcast=True)



@stream_routes.route("/broadcaster")
def broadcaster():
    return render_template("broadcaster.html")

@stream_routes.route("/viewer")
def viewer():
